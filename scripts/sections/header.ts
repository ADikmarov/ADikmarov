...
export async function buildHeader(): Promise<string> {
  const lines: Line[] = [
    ...
    { spans: [{ text: PROMPT, color: palette.green, bold: true }], class: 'out out3' },
  ];
...
<g class="out out3"><rect class="cursor" x="${cursorX}" y="${cursorTop}" width="${CHAR_W}" height="${LINE_H - 6}" fill="${palette.text}"/></g>
`;
